...
export function MenuBar({ 
  ...
  const appleMenuItems: MenuItem[] = [
    { label: 'About This Mac', action: () => window.dispatchEvent(new CustomEvent('openApp', { detail: 'about' })) },
    { separator: true },
    { label: 'System Settings...', shortcut: '⌘,', action: () => window.dispatchEvent(new CustomEvent('openApp', { detail: 'settings' })) },
    { label: 'App Store...', action: () => {} },
    { separator: true },
    { label: 'Recent Items', submenu: [
      { label: 'DeployHub Project', action: () => window.dispatchEvent(new CustomEvent('openApp', { detail: 'projects' })) },
      { label: 'Resume.pdf', action: () => window.dispatchEvent(new CustomEvent('openApp', { detail: 'resume' })) },
      { label: 'Clear Menu', action: () => {} },
    ]},
    { separator: true },
    { label: 'Force Quit...', shortcut: '⌘⌥⎋', action: () => {} },
    { separator: true },
    { label: 'Sleep', icon: <MoonIcon className="w-4 h-4" />, action: handleSleep },
    { label: 'Restart...', icon: <RefreshCw className="w-4 h-4" />, action: handleRestart },
    { label: 'Shut Down...', icon: <Power className="w-4 h-4" />, action: handleShutdown },
    { separator: true },
    { label: 'Lock Screen', shortcut: '⌘⌃Q', icon: <Lock className="w-4 h-4" />, action: handleLock },
    { label: 'Log Out Sudhanshu...', shortcut: '⇧⌘Q', action: () => {} },
  ];

  const fileMenuItems: MenuItem[] = [
    { label: 'New Finder Window', shortcut: '⌘N', action: () => window.dispatchEvent(new CustomEvent('openApp', { detail: 'finder' })) },
    { label: 'New Folder', shortcut: '⇧⌘N', action: () => {} },
    { label: 'New File', shortcut: '⌘T', icon: <FilePlus className="w-4 h-4" />, action: handleNewFile },
    { label: 'New Folder with Selection', shortcut: '⌃⌘N', action: () => {}, disabled: true },
    ...
    { separator: true },
    { label: 'Close Window', shortcut: '⌘W', action: () => window.dispatchEvent(new CustomEvent('closeActiveWindow')) },
  ...
  const windowMenuItems: MenuItem[] = [
    { label: 'Minimize', shortcut: '⌘M', action: () => window.dispatchEvent(new CustomEvent('minimizeActiveWindow')) },
    { label: 'Zoom', action: () => window.dispatchEvent(new CustomEvent('maximizeActiveWindow')) },
  ...
  const helpMenuItems: MenuItem[] = [
    { label: 'Portfolio Help', action: () => window.open('/rules.txt', '_blank') },
    { separator: true },
    { label: 'Keyboard Shortcuts', action: () => window.open('/rules.txt', '_blank') },
      ...
      {activeMenu === menuName && (
        ...
        >
          {items.map((item, index) => (
            item.separator ? (
              <div key={index} className={`h-px my-1 mx-2 ${isDark ? 'bg-white/10' : 'bg-black/10'}`} />
            ) : item.submenu ? (
              <div key={index} className="relative group">
                ...
                {/* Submenu */}
                <div className={`absolute left-full top-0 ml-0.5 min-w-[180px] rounded-lg overflow-hidden shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all ${
              ...
              <button
                ...
                className={`px-2 py-1 rounded text-sm transition-colors ${
                  activeMenu === menu 
                    ...
                    : (isDark ? 'text-white/80 hover:text-white hover:bg-white/10' : 'text-gray-700 hover:text-gray-900 hover:bg-black/10')
                }`}
              >
                {label}
              </button>
                ...
                >
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-4">
                      <span className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Wi-Fi</span>
                      <div className="w-11 h-6 bg-[#34c759] rounded-full relative">
                        <div className="absolute right-0.5 top-0.5 w-5 h-5 bg-white rounded-full shadow" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      {['Home WiFi', 'Office Network', 'iPhone Hotspot'].map((network, i) => (
                        <div key={network} className={`flex items-center justify-between p-2 rounded-lg ${
                          i === 0 
                            ? (isDark ? 'bg-[#007aff]/20' : 'bg-blue-50')
                            : (isDark ? 'hover:bg-white/5' : 'hover:bg-black/5')
                        }`}>
                          <div className="flex items-center gap-2">
                            <Wifi className={`w-4 h-4 ${i === 0 ? 'text-[#007aff]' : ''}`} />
                            <span className={isDark ? 'text-white/90' : 'text-gray-800'}>{network}</span>
                          </div>
                          {i === 0 && <Check className="w-4 h-4 text-[#007aff]" />}
                        </div>
                      ))}
                    </div>
                    <div className={`mt-4 pt-3 border-t ${isDark ? 'border-white/10' : 'border-black/10'}`}>
                      <button className={`text-sm ${isDark ? 'text-[#007aff]' : 'text-blue-600'}`}>
                        Network Preferences...
                      </button>
                    </div>
                  </div>
            ...
            <AnimatePresence>
              {showBluetoothPanel && (
                ...
                >
                  <div className="p-4">
                    <div className="flex items-center justify-between mb-4">
                      <span className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>Bluetooth</span>
                      <div className="w-11 h-6 bg-[#34c759] rounded-full relative">
                        <div className="absolute right-0.5 top-0.5 w-5 h-5 bg-white rounded-full shadow" />
                      </div>
                    </div>
                    ...
                    <div className={`mt-4 pt-3 border-t ${isDark ? 'border-white/10' : 'border-black/10'}`}>
                      <button className={`text-sm ${isDark ? 'text-[#007aff]' : 'text-blue-600'} w-full text-left`}>
        ...
        {isLocked && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-2xl font-semibold text-white mb-2">Sudhanshu Khosla</h2>
              <p className="text-white/60 mb-8">Click anywhere to unlock</p>
              <button
